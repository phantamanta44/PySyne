...
class Visualizer:
    ...
    def __init__(self, config, bar_window_max):
        ...
        self.bar_window = np.arange(0, bar_window_max, float(bar_window_max) / float(self.config.segs)) + 1
...
class CircleCurveVisualizer(Visualizer):
    ...
    def __init__(self, config):
        super().__init__(config, 10)
        self.path = Path()
        self.delta_angle = pi * 2 / self.config.segs

    def process(self, fft):
        fft = (fft * 5) * self.bar_window
        img = Image.new("RGB", self.size)
        offset_x, offset_y = map(lambda x: 0.5 * x, self.size)
        self.path.clear()
        for i in map(lambda n: n % self.config.segs, range(0, self.config.segs + 1)):
